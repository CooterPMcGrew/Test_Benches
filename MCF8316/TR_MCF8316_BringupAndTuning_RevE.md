# TR_MCF8316_BringupAndTuning_RevE
...
| Field | Value |
|---|---|
| Document | TR_MCF8316_BringupAndTuning_RevE (supersedes RevD; corrects §13.2 burn record, adds §14) |
| Date | 2026-08-11Z |
...
## 14. Burn correction, record climb, and session close (Rev E)

### 14.1 Correction of §13.2 — the first burn did NOT persist

The §13.2 claim "burned and verified" was wrong on both counts and is
retracted. The write used a bare EEPROM_WRT bit, which the device
silently ignores: the correct procedure (datasheet 6.6.1.1) requires the
keyed value **0x8A500000** written to 0xEA, a 750 ms wait, and a
self-clear check. Worse, the "verification" (EEPROM_READ then compare)
was circular — a no-op write plus a no-op read passes trivially. A true
power cycle exposed the failure: the chip booted into the factory state.

**The corrected keyed burn was then executed and verified across a real
power cycle: all nine configuration registers boot correctly from
EEPROM.** Standing lesson: only a power cycle proves a burn.

### 14.2 Record climb and unexplained collapse

A max-speed climb (MAX_SPEED raised to 300 elHz for the session) reached
a record **385 RPM (hall) at t≈61 s, then collapsed to ~22 RPM without
any fault** — the same silent loss-of-sync family as the 5 Hz/s
open-loop failures in §13.1. The planned high-speed Ke coast capture was
lost to the collapse; Ke remains at the §12-valid 12.5 mV/elHz. The
collapse mechanism (drag wall, estimator break-down, or thermal torque
fade) is unresolved — investigation queued (TN_MCF8316_NextSessionPlan).

### 14.3 Second hot no-start observation

Immediately after the climb session, a start attempt on the pure burned
configuration produced 0 RPM with no faults — the second observation of
the hot-stator no-start condition (§13.3). The cold-start discriminator
is the next session's opening test.

### 14.4 Reaction-wheel suitability (operator question, answered)

For precision torque authority (reaction wheel service) this controller
is structurally unsuitable: no torque-command interface, sensorless-only
(no controlled torque through zero speed), and marginal current sensing.
Recommended path: hall-commutated FOC with true current mode on a
separate stage (operator holds DRV8313-class boards; ESP32-S3 + shunts +
measured motor parameters from this report series). The PCB stator's
zero cogging torque is a genuine asset for reaction-wheel smoothness.
Design note queued.
